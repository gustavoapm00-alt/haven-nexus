...
import { useDeployAgentWorkflows, type StabilizationReport } from '@/hooks/useDeployAgentWorkflows';
...
function ReportBody({ report }: { report: StabilizationReport }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      {/* Metrics Bar */}
      <div
        className="px-4 py-4 flex items-center gap-10"
      ...
      <div
        className="px-4 py-3 flex items-center justify-between"
        style={{ borderTop: '1px solid rgba(57,255,20,0.05)', background: 'rgba(0,0,0,0.5)' }}
      >
        ...
        <span className="text-[7px]" style={{ fontFamily: MONO, color: '#1e2e1e' }}>
          [REF: COOP-PIPELINE-INTEGRITY-V1]
        </span>
      </div>
    </motion.div>
  );
...
export default function WorkflowStabilizationReport() {
  const { isLoading, error, stabilizationReport, phase, runStabilizationScan } = useDeployAgentWorkflows();
  ...
  return (
    <div
      className="w-full"
      style={{ border: '1px solid rgba(57,255,20,0.15)', background: 'rgba(0,0,0,0.9)', fontFamily: MONO }}
    >
      {/* Header */}
      <div
      ...
      >
        <div>
          ...
          <div className="text-[8px] tracking-[0.2em]" style={{ color: '#333' }}>
            ORPHAN_DETECTION // LATENCY_AUDIT // COOP_INTEGRITY
          </div>
        </div>
        <motion.button
      ...
      <AnimatePresence>
        {stabilizationReport && !isScanning && (
          <ReportBody report={stabilizationReport} />
        )}
      </AnimatePresence>
    </div>
  );
